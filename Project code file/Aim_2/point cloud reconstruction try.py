...
while True:
    ...
    if keypoints != None:
        w, h = color_image.shape[0], color_image.shape[1]
        for k in keypoints:
            x, y = k.pt
            x, y = int(x), int(y)
            ...
            depth_pixel = rs.rs2_project_color_pixel_to_depth_pixel(depth_frame.get_data(), depth_scale,
                                                                    0.11, 1.0, depth_intrin, color_intrin, depth_to_color_extrin, color_to_depth_extrin, [x,y])
            print(x,y)
            # print(a,b)
            if x >= 0 and x < w and y >= 0 and y < h:
                p = rs.rs2_deproject_pixel_to_point(depth_intrinsics, [x,y], depth_image[int(depth_pixel[0]), int(depth_pixel[1])]*depth_scale)
